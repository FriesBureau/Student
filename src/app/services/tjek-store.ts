...
export class TjekStoreService {


  constructor(private tjekService: TjekService) {
    this.fetchAll()
  }
  ...
  private readonly _todos = new BehaviorSubject<Tjek[]>([]);
  ...
  async addTodo(title: string) {
    ...
    if(title && title.length) {
      ...
      try {
        const todo = await this.tjekService
  ...
  async removeTodo(id: string, serverRemove = true) {
    ...
    if(serverRemove) {
      try {
        await this.tjekService.remove(id).toPromise();
      } catch (e) {
        console.error(e);
  ...
  async fetchAll() {
    this.todos = await this.tjekService.index().toPromise();
  }
