...
import { RecognitionResult } from '../assistent-speech-support.service';



@Component({
  selector: 'app-assistent-yes-no-dialog',
  template: `
...
export class AssistentPracticeDialogComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<AssistentPracticeDialogComponent>,
...
//    this.confidence = this.data.confidence * 100;
  ...
  public chooseYes(){
    ...
    this.dialogRef.close({choice:AssistentYesNo.yes, response:this.data.transcript})
  }
  public chooseAlternative(){
    console.log('click on alternative.');
    this.dialogRef.close({choice:AssistentYesNo.alternative, response:this.data.alternative})
  }

  public chooseNo(){
    console.log('click on no.');
    this.dialogRef.close({choice:AssistentYesNo.no, response:this.data});
  }
}
