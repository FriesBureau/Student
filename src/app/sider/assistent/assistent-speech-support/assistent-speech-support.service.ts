...
import { AssistentYesNo } from './assistent-yes-no-dialog/assistent-yes-no.enum';
...
export class AssistentSpeechSupportService {
  ...
  private handleResultevent(event: any): void {
    ...
    if (this._lastResult.transcript.includes('Det er ikke godt')) {  
console.log('Lyd passer på ord');
console.log('confidence ' + this._lastResult.confidence);

if( typeof(this._lastResult.transcript) === 'string' ) { console.log('i service er string') 
} else {
  console.log('i service er ikke string') 
}
    } else {
console.log('Lyd passer ikke på ord');
   for (var i = 0; i < resultlist.length; ++i) {
    console.log('Resultat ' + this._lastResult.transcript);
   this._lastResult.alternative = resultlist[i].transcript;
   console.log('Alternativ ' + this._lastResult.alternative);
    }   
    }
  /* Virker ikke - Giver lastResult[object Object] 
  ...
  */
  ...
  private showDialogPractice (recognitionResult: RecognitionResult): void {
    this.zone.run(() => {
      const dialogRef = this.dialog.open(AssistentPracticeDialogComponent, {
        width: '600px',
        height: '350px',
      ...
      dialogRef.afterClosed().subscribe(result => {
        ...
        console.log(result);
        if (result.choice === AssistentYesNo.yes) {
          this.Result.emit(result.response);
        }
        else if (result.choice === AssistentYesNo.alternative) {
          this.Result.emit(result.response);
  ...
  private showDialog(recognitionResult: RecognitionResult): void {
    ...
    this.zone.run(() => {
      const dialogRef = this.dialog.open(AssistentYesNoDialogComponent, {
        width: '500px',
        height: '300px',
        data: recognitionResult,
        id: 'yesnodlg'
